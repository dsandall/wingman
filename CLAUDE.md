...
- `packaging/arch/` — AUR package, pacman capability hook, wheel-group DNS rule
- `packaging/debian/` + `debian/` — native Debian package, dpkg path trigger, sudo-group DNS rule
...
- The recommended Linux setup is **rootless**: the daemon runs as the user (config under `~/.config/wingman`, no `sudo` for everyday commands), with file capabilities granted to the `netbird` binary — `CAP_NET_ADMIN` (create the WireGuard interface) and `CAP_NET_BIND_SERVICE` (bind the DNS resolver to port 53; see the DNS note). Both are granted in one `setcap cap_net_admin,cap_net_raw,cap_net_bind_service+eip`. `instance.py` preflights `CAP_NET_ADMIN` (`_require_kernel_iface_capability`) and aborts a rootless `up` with the exact `setcap` command when it's missing. The Arch package restores these after NetBird upgrades with a pacman hook; the Debian package does so with a dpkg path trigger on `/usr/bin/netbird`.
- **DNS** is entirely NetBird's job — wingman has no DNS code; it only keeps each instance's interface/state separate so NetBird's per-interface DNS doesn't collide. NetBird binds its resolver to each instance's interface IP and registers it per-link with systemd-resolved using `~`-prefixed match domains (split DNS), so instances coexist without a global resolver takeover. Rootless DNS first needs **systemd-resolved running and owning `/etc/resolv.conf`** (stub symlink or a copy naming 127.0.0.53): a non-root daemon can't rewrite resolv.conf, so on a NetworkManager-only host NetBird's host-DNS detection falls back to the `file` manager, logs `failed to apply DNS host manager update … permission denied`, and names silently never resolve even with both grants below in place (`resolvectl status` shows no `wt*` links at all). Then it needs **two** grants, both non-fatal if missing (the tunnel still works; only name resolution breaks): (1) **`CAP_NET_BIND_SERVICE`** so the resolver can bind **port 53** on the interface IP — resolved sends queries to `<iface-ip>:53`, but a rootless daemon that can't bind 53 falls back to `:5053`, which resolved never queries, so lookups get connection-refused even though `resolvectl status` shows DNS configured; and (2) a **polkit rule** authorizing the daemon's resolved calls (`org.freedesktop.resolve1.set-*`), without which resolved ignores the resolver entirely (`Current Scopes: none`, plus a `failed to apply DNS host manager update … requires interactive authentication` line in `daemon.log`). The polkit fix is `packaging/arch/wingman-netbird-dns.rules` (authorizing the `wheel` group) installed to `/usr/share/polkit-1/rules.d/`. `instance.py` preflights all three (`_warn_dns_unavailable`: `platform.resolved_dns_state` — `systemctl is-active` plus a resolv.conf ownership check — for resolved, `pkcheck` for polkit, `getcap` for the bind cap) and **warns without blocking** on a rootless `up`. The resolved fix (`systemctl enable --now systemd-resolved` + `ln -sf /run/systemd/resolve/stub-resolv.conf /etc/resolv.conf`) is host setup the package can't do for you. Caveat: separate NetBird accounts share the `netbird.cloud` zone, so their match domains overlap — fine unless a peer hostname is duplicated across accounts
